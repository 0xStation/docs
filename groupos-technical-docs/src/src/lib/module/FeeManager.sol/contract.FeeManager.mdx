# FeeManager
[Git Source](https://github.com/0xStation/groupos/blob/dbf5849fa48aac03711471c1ce33b8c26f177a2c/src/lib/module/FeeManager.sol)
...
*This contract stores state for all fees set on both a one-size fits all default basis and per-collection basis
...
```solidity
function getFeeTotals(address collection, address paymentToken, address recipient, uint256 quantity, uint256 unitPrice)
    external
    view
...
|`recipient`|`address`|The address to mint to. Checked to apply discounts per user for Station Network incentives|

# MetadataRouter
[Git Source](https://github.com/0xStation/groupos/blob/dbf5849fa48aac03711471c1ce33b8c26f177a2c/src/metadataRouter/MetadataRouter.sol)
...
The number of elements in `routes` and `routeURIs` arrays must match, or initialization will revert.

The contract owner will have exclusive rights to manage metadata routes and URIs.

*Initialize the contract with default URIs and ownership information.*
...
|`defaultURI_`|`string`|The default URI to be used when no specific URI is configured.|
|`routes`|`string[]`|An array of route names.|
|`routeURIs`|`string[]`|An array of URIs corresponding to the routes provided.|
...
|`contractAddress`|`address`|The address of the contract for to request a URI.|
